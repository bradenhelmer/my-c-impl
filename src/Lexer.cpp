...
void Lexer::lexIdentifier() {
  ...
  if (isTypeKeyword(ident)) {
    currentToken.kind = getTypeKeywordToken(ident);
  }
}
...
std::string Lexer::getIdentifier() {
  return std::string(currentToken.start, currentToken.end + 1);
}

double Lexer::getNumericLiteral() {
  return 
      std::stod(std::string(currentToken.start, currentToken.end + 1));
}
